-- A manually chosen destination is a group decision, not a KREW-scored recommendation.
-- Preserve the existing presentation logic for generated recommendations while
-- short-circuiting cards explicitly marked with budget.manualChoice.
...
begin
  ...
  if coalesce((new.budget->>'manualChoice')::boolean, false) then
    new.match_reasons := array['Choisie par le groupe']::text[];
    new.rationale := 'Destination renseignée manuellement par le groupe.';
    return new;
  end if;
