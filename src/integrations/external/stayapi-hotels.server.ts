...
export async function resolveStayApiDestination(destination: string): Promise<{ id: string; type: string }> {
  ...
  if (!candidates.length) {
    throw new Error(`StayAPI: aucun dest_id trouvé pour la destination "${destination}"`);
  }

  const normDest = destination.trim().toLowerCase();
  const exact = candidates.find((d: any) => {
    const label = String(d?.name ?? d?.label ?? d?.city_name ?? d?.dest_name ?? d?.title ?? "").trim().toLowerCase();
    return label === normDest;
  });

  const city = candidates.find((d: any) => {
    const typeStr = String(d?.dest_type ?? d?.type ?? d?.search_type ?? "").trim().toUpperCase();
    return typeStr === "CITY";
  });
  ...
  if (!id) {
    throw new Error(`StayAPI: dest_id introuvable pour "${destination}" dans la réponse lookup`);
  }

  const rawType = String(selected?.dest_type ?? selected?.type ?? "CITY").trim().toUpperCase();
  const type = rawType || "CITY";
...
export async function searchHotelsStayApi(params: SearchParams & { destId?: string; destType?: string }): Promise<HotelOffer[]> {
  ...
  if (!destId) {
    throw new Error(`StayAPI: dest_id manquant pour la destination "${params.destination}"`);
  }
  ...
  const hotels = pickArray(body)
    .map(normalizeHotel)
    .filter((hotel): hotel is HotelOffer => Boolean(hotel));

  if (!hotels.length) {
    throw new Error(`StayAPI: aucun hôtel trouvé pour dest_id "${destId}" (${params.destination})`);
  }
