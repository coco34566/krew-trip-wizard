...
export async function fetchClimate(
  ...
  if (res.ok) {
    ...
    const buckets = new Map<number, { maxT: number[]; minT: number[]; p: number; years: Set<number> }>();
    time.forEach((day, i) => {
      const month = Number(day.slice(5, 7));
      const year = Number(day.slice(0, 4));
      const bucket = buckets.get(month) ?? { maxT: [], minT: [], p: 0, years: new Set<number>() };
      const maxT = maxTemps[i];
      const minT = minTemps[i];
      const p = rain[i];
      if (typeof maxT === "number") bucket.maxT.push(maxT);
      if (typeof minT === "number") bucket.minT.push(minT);
      if (typeof p === "number") bucket.p += p;
      bucket.years.add(year);
      buckets.set(month, bucket);
    });
    for (const [month, bucket] of [...buckets.entries()].sort((a, b) => a[0] - b[0])) {
      if (!bucket.maxT.length) continue;
      months.push({
        month,
        tempMaxAvg: Math.round((bucket.maxT.reduce((a, b) => a + b, 0) / bucket.maxT.length) * 10) / 10,
        tempMinAvg: bucket.minT.length
          ? Math.round((bucket.minT.reduce((a, b) => a + b, 0) / bucket.minT.length) * 10) / 10
          : undefined,
