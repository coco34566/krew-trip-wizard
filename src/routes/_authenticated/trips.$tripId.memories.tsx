...
function MemoriesPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => { toast.error("Impossible de supprimer la photo."); console.error(e); },
  });
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (err) { toast.error("Impossible d'importer les photos."); console.error(err); }
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-10 space-y-8 relative">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-2"><div className="flex items-center gap-2 text-primary"><Camera className="size-6" /><span className="text-xs font-semibold uppercase tracking-wider">Souvenirs</span></div><h1 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">L&apos;album de notre voyage</h1><p className="text-sm text-muted-foreground max-w-xl">Retrouve ici les meilleurs moments partagés avec tes ami·e·s. Ajoute tes propres pépites et vote pour tes préférées !</p></div>
        {permission !== "prompt" && <Button variant="outline" size="sm" onClick={resetPermission} className="gap-1.5 text-xs"><Settings className="size-3.5" /> Gérer l&apos;autorisation</Button>}
      </header>
      <section className="rounded-3xl border border-dashed border-border bg-surface/30 p-8 text-center space-y-3 relative overflow-hidden">
        ...
        <div className="mx-auto w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary"><Plus className="size-6" /></div>
        <div><p className="font-semibold text-sm">Ajoute tes photos de voyage</p><p className="text-xs text-muted-foreground mt-0.5">Tes photos sont stockées dans un espace privé et partagées uniquement avec les participants du voyage.</p></div>
        {permission === "denied" && <div className="mx-auto max-w-md bg-destructive/10 border border-destructive/20 text-destructive rounded-2xl p-3 flex items-start gap-2 text-left text-xs"><ShieldAlert className="size-4 shrink-0 mt-0.5" /><div><p className="font-semibold">L&apos;accès aux photos a été désactivé</p><p className="mt-0.5">Tu peux modifier ce choix avec « Gérer l&apos;autorisation ».</p></div></div>}
        <Button size="sm" className="gap-1.5" disabled={uploading} onClick={handleImportClick}>{uploading ? <><Loader2 className="size-3.5 animate-spin" /> Importation...</> : <><Sparkles className="size-3.5" /> Choisir des photos</>}</Button>
      </section>
