  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button variant="ghost" size="icon" className={compact ? "size-8 rounded-full bg-background/90 backdrop-blur-sm text-muted-foreground hover:text-destructive" : "size-9 rounded-full text-muted-foreground hover:text-destructive"} aria-label="Archiver le voyage">
    ...
    <article className="relative isolate min-h-[500px] sm:min-h-[485px] lg:min-h-[510px] overflow-visible">
      ...
      <div className="absolute right-[1%] top-[260px] sm:top-[92px] w-[61%] sm:w-[43%] lg:w-[42%] z-20 px-2 sm:px-4">
        ...
        <div className="flex items-start justify-between gap-2">
          <div><p className="text-[10px] sm:text-xs font-mono uppercase tracking-[.12em] text-primary">J'organise</p><h2 className="mt-1 font-display text-[28px] sm:text-[38px] lg:text-[44px] leading-[.95] font-normal text-foreground">{trip.name}</h2></div>
      ...
      <div className="relative bg-[#fffefa] p-2 pb-4 shadow-[0_11px_24px_-18px_rgba(42,25,37,.26)] ring-1 ring-black/[.05] transition-transform duration-200 group-hover:-translate-y-1">
        ...
        <Link to="/trips/$tripId" params={{ tripId: trip.id }} className="block">
          ...
          <div className="px-1.5 pt-3">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-2.5 gap-y-1 text-[9px] sm:text-[10px] text-muted-foreground">
              <span className="font-mono uppercase tracking-[.06em]">{eventTypeLabel(trip.event_type)}</span>
              {trip.destination_name ? <span className="inline-flex items-center gap-1"><KrewIcon name="destination" tone="muted" size="sm" className="size-3" />{trip.destination_name}</span> : null}
              <span className="inline-flex items-center gap-1"><KrewIcon name="group" tone="muted" size="sm" className="size-3" />{trip.participants_count}</span>
            </div>
          </div>
        </Link>
        ...
        <div className="mx-1.5 mt-2.5 flex items-center justify-between gap-2 border-t border-dashed border-sage/30 pt-2">
          ...
          <Link to="/trips/$tripId" params={{ tripId: trip.id }} className="relative z-10 inline-flex shrink-0 items-center gap-1 text-[11px] font-semibold text-primary">{invited ? "Voir" : "Continuer"}<KrewMark type="arrow-right" tone="plum" size="sm" className="h-3.5 w-5" /></Link>
        </div>
      </div>
    </article>
  );
