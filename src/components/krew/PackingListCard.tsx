      ...
      {items.map((item) => {
        const link =
          item.purchasable && !state.owned[item.id]
            ...
            : null;
        const assignedParticipant = assignableParticipants.find((participant) => participant.id === state.assigned[item.id]);
        const assigneeLabel = state.owned[item.id]
          ? "Pris en charge"
          : assignedParticipant
            ? assignedParticipant.display_name || assignedParticipant.email?.split("@")[0] || "Participant"
            : "Non attribué";
        return (
          <li key={item.id} className="py-3 flex flex-col gap-2">
            <div className="flex items-start gap-2.5">
              {historical ? (
                <span className="-ml-1 -mt-1 shrink-0 text-muted-foreground min-h-11 min-w-11 inline-flex items-center justify-center rounded-lg" aria-hidden="true">
            ...
            {group ? (
              <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2 pl-0 sm:pl-10">
                {historical ? (
                  <span className="text-sm text-muted-foreground">Responsable : {assigneeLabel}</span>
                ) : (
                  <select
