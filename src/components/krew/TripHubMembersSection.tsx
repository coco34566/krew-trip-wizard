...
import type { useTripHubActions } from "@/hooks/useTripHubActions";
...
type HubActions = ReturnType<typeof useTripHubActions>;
...
type TripWithLegacyCoOrganizer = Tables<"trips"> & {
...
export function TripHubMembersSection({
  ...
  trip: TripWithLegacyCoOrganizer;
  participants: TripHubMember[];
  logistics: { transportPicks?: TransportPick[] };
  progress: TripHubProgress | undefined;
  ...
  const buildWhatsAppRemindMessage = () => {
    ...
    for (const participant of missingParticipants) {
      const name = participant.display_name || participant.email?.split("@")[0] || "Ami";
  ...
  const coOrganizerId = trip.co_organizer_id || trip.coOrganizerId;
      ...
      <ul className="divide-y divide-border/40 pt-1">
        {participants.length === 0 ? (
          ...
          participants.map((participant) => {
            const picks = logistics.transportPicks ?? [];
            const userPick = participant.user_id
              ? picks.find((pick) => pick.userId === participant.user_id)
              : null;
            const city =
              progress?.participants?.find((item) => item.user_id === participant.user_id)
                ?.departure_city ||
              participant.departure_city ||
              userPick?.city ||
              null;
            const memberIsOwner = Boolean(
              participant.user_id &&
                !participant.placeholder &&
                participant.user_id === trip.owner_id,
            );
            const memberIsCoOrganizer = Boolean(
              participant.user_id &&
                !participant.placeholder &&
                participant.user_id !== starUid &&
                participant.user_id !== trip.owner_id &&
                participant.user_id === coOrganizerId,
            );
            ...
            const initial = String(
              participant.display_name || participant.email || "P",
            )
              .trim()
              .charAt(0)
              .toUpperCase();
                ...
                <div className="flex items-center gap-2 sm:gap-3 shrink-0 text-xs">
                  ...
                  {participant.user_id === viewerUserId ? (
                    <button
                      ...
                      onClick={() => {
                        const nextStatus =
                          participant.status === "absent" ? "accepte" : "absent";
                  ...
                  participant.user_id !== trip.owner_id ? (
                    participant.user_id === coOrganizerId ? (
                      <button
                        ...
                        onClick={() =>
                          actions.setCoOrgMutation.mutate({ coOrganizerId: null })
                        }
                      >
            ...
            isEditingCount ? (
              <form
                onSubmit={(event) => {
                  ...
                  if (countInput >= 2 && countInput <= 25) {
                    actions.updateCountMutation.mutate(countInput, {
              ...
              >
                <Input
                  ...
                  onChange={(event) => setCountInput(Number(event.target.value))}
      ...
      {isOwner ? (
        <div className="pt-4 border-t border-border/40">
          <Button
            ...
            onClick={() => shareOnWhatsApp(buildWhatsAppRemindMessage())}
          >
