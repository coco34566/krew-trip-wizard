...
export function TripHubMembersSection({
  ...
  setIsEditingCount: (value: boolean) => void;
  countInput: number;
  setCountInput: (value: number) => void;
  actions: any;
  buildWhatsAppRemindMessage: () => string;
}) {
  return (
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 shrink-0 text-xs">
                        ...
                        {data.isCreator && p.user_id && !p.placeholder && !p.isStar && p.user_id !== "star-virtual-uid" && p.user_id !== trip.owner_id ? (
                          p.user_id === (trip.co_organizer_id || (trip as any).coOrganizerId) ? (
                            ...
                            <button
                              ...
                              disabled={actions.setCoOrgMutation.isPending}
                              onClick={() =>
                                actions.setCoOrgMutation.mutate({ coOrganizerId: p.user_id || null })
                              }
                            >
                        ...
                        {data.isOwner && !p.placeholder && !p.isStar ? (
                          <button
                            ...
                            onClick={() => actions.removeMutation.mutate(p.id)}
                          >
