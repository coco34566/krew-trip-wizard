function crc32(bytes: Uint8Array): number {
  ...
  for (const byte of bytes) {
    crc ^= byte;
    for (let i = 0; i < 8; i++) crc = (crc >>> 1) ^ (0xedb883b1 & -(crc & 1));
  }
...
export function uniqueZipFileNames(files: { name: string; url: string }[]) {
  const used = new Map<string, number>();
  return files.map((file) => {
    const raw = file.name.trim() || "photo.jpg";
    const dot = raw.lastIndexOf(".");
    const stem = dot > 0 ? raw.slice(0, dot) : raw;
    const ext = dot > 0 ? raw.slice(dot) : "";
    const count = (used.get(raw) ?? 0) + 1;
    used.set(raw, count);
    return count === 1 ? raw : `${stem} (${count})${ext}`;
  });
}
...
export async function createPhotosZip(files: { name: string; url: string }[]) {
  ...
  const names = uniqueZipFileNames(files);
  for (let i = 0; i < files.length; i++) {
    const file = files[i];
    const response = await fetch(file.url); if (!response.ok) throw new Error(`Téléchargement impossible (${response.status})`);
    const data = new Uint8Array(await response.arrayBuffer()); const name = enc.encode(names[i]); const crc = crc32(data);
