...
export async function refreshExternalCatalogForTrip(
  ...
  // Le budget est un signal de classement et d'avertissement, jamais un veto individuel.
  // Conserver les offres au-dessus de la cible permet au moteur de les pénaliser sans qu'une
  // seule réponse de questionnaire fasse disparaître une option pertinente pour le groupe.
  ...
  const overTargetCount = targetStayPerPerson > 0
    ? hotels.filter((hotel) => (hotel.offers[0]?.perPersonStay ?? Infinity) > targetStayPerPerson).length
    : 0;
  ...
  if (hotels.length) {
    const rows = hotels.map((h) => {
      ...
      const priceVerified = Boolean(best && best.estimated !== true);
      const availabilityVerified = Boolean(checkin && checkout);
      return {
        destination_id: destinationId,
        name: h.name,
        type: h.type || "other",
        description: h.description,
        price_per_night_per_person: Math.max(1, Math.round(best?.perPersonPerNight ?? 0)),
        ...
        verification_state: priceVerified && availabilityVerified ? "confirmed" : "inferred",
      };
    });
