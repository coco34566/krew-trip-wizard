...
async function resolveStayApiDestination(destination: string) {
  // StayAPI's documented destination resolver is /booking/destinations/lookup.
  // It accepts the human-readable KREW destination and returns the Booking.com
  // destination ID needed by /booking/search.
  const payload = await stayApi("/v1/booking/destinations/lookup", {
    query: destination,
    language: "fr",
  });
  ...
  const candidates = [
    ...
    ...(Array.isArray(payload?.data?.destinations) ? payload.data.destinations : []),
  ];

  const normalizedDestination = destination.normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().toLowerCase();
  const exact = candidates.find((d: any) => {
    const name = String(d?.name ?? d?.label ?? d?.city_name ?? d?.destination_name ?? "")
      .normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().toLowerCase();
    return name === normalizedDestination;
  });
  const city = candidates.find((d: any) => String(d?.geo_type ?? d?.type ?? d?.dest_type ?? "").toLowerCase() === "city");
  const d = exact ?? city ?? candidates[0];
  const id = d?.dest_id ?? d?.destination_id ?? d?.destinationId ?? d?.id;
  if (!id) throw new Error(`StayAPI: destination ID introuvable pour "${destination}"`);
  return { id: String(id), type: String(d?.dest_type ?? d?.geo_type ?? d?.type ?? "CITY").toUpperCase() };
}
