...
export const CITY_IATA: Record<string, string> = {
  paris: "PAR", "paris cdg": "CDG", "paris orly": "ORY", lyon: "LYS", marseille: "MRS", nice: "NCE",
  toulouse: "TLS", bordeaux: "BOD", nantes: "NTE", lille: "LIL", strasbourg: "SXB", montpellier: "MPL",
  rennes: "RNS", geneve: "GVA", geneva: "GVA", bruxelles: "BRU", brussels: "BRU", amsterdam: "AMS",
  londres: "LON", london: "LON", barcelone: "BCN", barcelona: "BCN", madrid: "MAD", rome: "ROM", roma: "ROM",
  milan: "MIL", lisbonne: "LIS", lisbon: "LIS", porto: "OPO", berlin: "BER", munich: "MUC", "münchen": "MUC",
  francfort: "FRA", frankfurt: "FRA", vienne: "VIE", vienna: "VIE", prague: "PRG", budapest: "BUD",
  cracovie: "KRK", varsovie: "WAW", dubrovnik: "DBV", split: "SPU", athenes: "ATH", athens: "ATH",
  dublin: "DUB", copenhague: "CPH", stockholm: "STO", oslo: "OSL", zurich: "ZRH", basel: "BSL", bale: "BSL",
};

/** Coordonnées approx pour estimer distance train vs avion. */
const CITY_COORDS: Record<string, { lat: number; lon: number }> = {
  paris: { lat: 48.86, lon: 2.35 }, lyon: { lat: 45.75, lon: 4.85 }, marseille: { lat: 43.3, lon: 5.4 },
  nice: { lat: 43.7, lon: 7.27 }, toulouse: { lat: 43.6, lon: 1.44 }, bordeaux: { lat: 44.84, lon: -0.58 },
  nantes: { lat: 47.22, lon: -1.55 }, lille: { lat: 50.63, lon: 3.06 }, strasbourg: { lat: 48.57, lon: 7.75 },
  montpellier: { lat: 43.61, lon: 3.88 }, rennes: { lat: 48.11, lon: -1.68 }, geneve: { lat: 46.2, lon: 6.14 },
  bruxelles: { lat: 50.85, lon: 4.35 }, amsterdam: { lat: 52.37, lon: 4.9 }, londres: { lat: 51.51, lon: -0.13 },
  barcelone: { lat: 41.39, lon: 2.17 }, madrid: { lat: 40.42, lon: -3.7 }, rome: { lat: 41.9, lon: 12.5 },
  milan: { lat: 45.46, lon: 9.19 }, lisbonne: { lat: 38.72, lon: -9.14 }, berlin: { lat: 52.52, lon: 13.4 },
  munich: { lat: 48.14, lon: 11.58 }, vienne: { lat: 48.21, lon: 16.37 }, prague: { lat: 50.08, lon: 14.44 },
...
export function buildOriginDeepLinks(ctx: DeepLinkContext): OriginDeepLinks {
  ...
  const ret = ctx.returnDate && /^\d{4}-\d{2}-\d{2}/.test(ctx.returnDate) ? ctx.returnDate.slice(0, 10) : addDays(depart, nights);
  ...
  if (showTrain) {
    let omioUrl = `https://www.omio.fr/search?departurePosition=${encodeURIComponent(originLabel)}&arrivalPosition=${encodeURIComponent(destLabel)}&departureDate=${depart}&returnDate=${ret}&adults=${adults}`;
    omio = appendAffiliateParam(omioUrl, "partner_id", "OMIO_AFFILIATE_ID");
    trainline = `https://www.thetrainline.com/search/${encodeURIComponent(originLabel)}/${encodeURIComponent(destLabel)}/${depart}/${ret}`;
    sncf = `https://www.sncf-connect.com/app/home/search/?originLabel=${encodeURIComponent(originLabel)}&destinationLabel=${encodeURIComponent(destLabel)}&outwardDate=${depart}&inwardDate=${ret}&passengers=${adults}`;
  }
...
export function buildDeepLinksForProposal(params: {
  ...
  nights?: number;
  fallbackDistanceKm?: number | null;
  /** Nombre total de voyageurs du groupe pour les liens d'hébergement. */
  groupAdults?: number;
}): { origins: OriginDeepLinks[]; bookingGroup: string } {
  const groupAdults = Math.max(1, Math.min(30, Number(params.groupAdults ?? 0) || params.origins.reduce((s, o) => s + Math.max(0, Number(o.count) || 0), 0) || 1));

  // Si aucune origine connue n'est disponible, conserver le fallback historique Paris,
  // mais ne plus afficher artificiellement 1 voyageur pour un groupe plus grand.
  const origins = (params.origins.length ? params.origins : [{ city: "Paris", count: groupAdults }]).map((o) => {
    const dLinkCtx: DeepLinkContext = {
      originCity: o.city,
      destinationCity: params.destinationCity,
      departDate: params.departDate,
      returnDate: params.returnDate,
      adults: Math.max(1, Number(o.count) || 1),
    };
  ...
  const ret = params.returnDate && /^\d{4}-\d{2}-\d{2}/.test(params.returnDate) ? params.returnDate.slice(0, 10) : addDays(depart, nights);
  let bookingGroup = `https://www.booking.com/searchresults.html?ss=${encodeURIComponent(params.destinationCity)}&checkin=${depart}&checkout=${ret}&group_adults=${groupAdults}&no_rooms=${Math.max(1, Math.ceil(groupAdults / 2))}&lang=fr`;
