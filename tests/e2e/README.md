...
The Playwright global setup authenticates with `KREW_E2E_EMAIL` and removes only test-prefixed trips owned by that account that are older than 24 hours. Global teardown removes only test-prefixed trips tagged with the current run token. Every delete is additionally constrained by trip id, exact owner id and exact trip name, and relies on the authenticated Supabase session/RLS rather than a service-role key. Cleanup failures are warnings and never change the test result.
