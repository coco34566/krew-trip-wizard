...
async function dismissCookieBanner(page: Page) {
  ...
  if (await reject.isVisible().catch(() => false)) { await reject.click(); return; }
...
export function installDiagnostics(page: Page, testInfo: TestInfo) {
  ...
  page.on("requestfailed", (request) => {
    ...
    if (isExpectedAbortedNavigation(request.url(), errorText)) { ignoredAbortedRequests.push(entry); return; }
  ...
  return async () => {
    await testInfo.attach("browser-diagnostics", {
      body: Buffer.from(JSON.stringify({ consoleErrors, failedRequests, ignoredAbortedRequests, paidProviderRequests }, null, 2)),
